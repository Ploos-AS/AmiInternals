#!/usr/bin/env python3
"""Local, visible AmigaOS 1.2 qualification. Never fetches system media."""
import argparse
import hashlib
import json
import os
...
def validate(out, tool):
    ...
    assert tool + ' 0.1\n' in text and 'AmiInternals - Ploos AS' in text
    if tool == 'Info':
        assert 'Exec ............ 33.' in text
        values = list(map(int, re.findall(r'(\d+) bytes', text)))
        assert len(values) == 6
        free, largest, fast, fast_largest, total, biggest = values
        assert 0 < largest <= free <= 524288
        assert fast == fast_largest == 0
        assert total == free and biggest == largest
    elif tool == 'Mem':
        values = list(map(int, re.findall(r'(\d+) bytes', text)))
        assert len(values) == 9
        for free, largest, fragmented in [values[i:i+3] for i in (0, 3, 6)]:
            assert 0 <= largest <= free <= 524288
            assert fragmented == free - largest
        assert values[3:6] == [0, 0, 0]
        assert values[0:3] == values[6:9] and values[0] > 0
        assert all('\n'+group+'\n' in text for group in ['Chip', 'Fast', 'All'])
    else:
        assert 'State Pri Name\n' in text
        rows = text.split('State Pri Name\n', 1)[1].strip().splitlines()
        assert rows and any(row.startswith('RUN ') for row in rows)
        for row in rows:
            match = re.fullmatch(r'(RUN|READY|WAIT) +(-?\d+) (.+)', row)
            assert match and -128 <= int(match[2]) <= 127, row
            assert all(32 <= ord(c) < 127 for c in match[3]), row
    return 'PASS'


def screenshot(path):
    """Capture only the emulator window's composited pixmap under Xwayland."""
    import ctypes as c
    ...
    if not im:
        raise RuntimeError('Cannot capture the emulator pixmap')
    rgb = bytearray()
...
def main():
    ...
    parser.add_argument('--tool', choices=['Info', 'Mem', 'Tasks'])
    parser.add_argument('--out', type=Path, required=True)
    parser.add_argument('--seconds', type=int, default=65)
    args = parser.parse_args()
    if args.tool is None:
        for tool in ['Info', 'Mem', 'Tasks']:
        ...
        (args.out / 'result.txt').write_text('Q1: PASS — real Kickstart 1.2 + Workbench/AmigaDOS 1.2\n')
        return
    ...
    # Copyrighted system media stays outside the checkout, even in ignored paths.
    import tempfile
    private = Path(tempfile.mkdtemp(prefix='amiinternals-q1-', dir='/tmp'))
    disk = private / 'workbench.adf'
    shutil.copyfile(wb, disk)
    command(xdf, disk, 'makedir', 'Q1')
    binary = root / 'build' / args.tool
    command(xdf, disk, 'write', binary, 'Q1/' + args.tool)
    extracted = private / args.tool
    command(xdf, disk, 'read', 'Q1/' + args.tool, extracted)
    assert sha(binary) == sha(extracted), 'Guest binary differs from host binary'
    status = private / 'Q1Status'
    ...
    command(xdf, disk, 'write', status, 'Q1/Q1Status')
    sequence = out / 'startup-sequence'
    sequence.write_text('FailAt 1\n'
        'Echo "AmigaOS 1.2 Q1: ' + args.tool + '"\n'
        'SYS:Q1/' + args.tool + ' >SYS:Q1/output.txt\n'
        'SYS:Q1/Q1Status >SYS:Q1/status.txt\n'
        'Echo >SYS:Q1/returned.txt "RETURNED BELOW FAILAT 1"\n'
        'Type SYS:Q1/output.txt\n'
        'Type SYS:Q1/status.txt\n'
        'Echo "Q1 command returned normally"\n')
    ...
    for name in ['output.txt', 'returned.txt', 'status.txt']:
        try:
            command(xdf, disk, 'read', 'Q1/' + name, out / name)
