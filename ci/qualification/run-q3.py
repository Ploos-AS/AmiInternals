#!/usr/bin/env python3
"""Local, visible AmigaOS 1.2 Q3 qualification. Never fetches system media."""
import argparse
import hashlib
import json
import os
...
def validate(out, tool):
    ...
    if tool == 'Resources':
        ...
        for row in rows:
            assert len(row) <= 63, row
            assert all(32 <= ord(c) < 127 for c in row), row
    else:
        assert 'Ver Type Pri Name\n' in text
        body = text.split('Ver Type Pri Name\n', 1)[1]
        rows = [r for r in body.splitlines() if r and not r.startswith('Warning:')]
        assert rows, 'No resident modules listed'
        for row in rows:
            m = re.fullmatch(r'(\d+) (\d+) (-?\d+) (.+)', row)
            assert m, row
            assert 0 <= int(m.group(1)) <= 255, row
            assert 0 <= int(m.group(2)) <= 255, row
            assert -128 <= int(m.group(3)) <= 127, row
            assert len(m.group(4)) <= 63, row
            assert all(32 <= ord(c) < 127 for c in m.group(4)), row
...
def main():
    ...
    if args.tool is None:
        ...
        (args.out / 'result.txt').write_text('Q3: PASS — real Kickstart 1.2 + Workbench/AmigaDOS 1.2\n')
        return
    ...
    private = Path(tempfile.mkdtemp(prefix='amiinternals-q3-', dir='/tmp'))
    disk = private / 'workbench.adf'
    shutil.copyfile(wb, disk)
    command(xdf, disk, 'makedir', 'Q3')
    binary = root / 'build' / args.tool
    if not binary.exists():
        raise SystemExit('BLOCKED: missing Q3 binary; run ci/qualification/build-q3.sh first')
    command(xdf, disk, 'write', binary, 'Q3/' + args.tool)
    extracted = private / args.tool
    command(xdf, disk, 'read', 'Q3/' + args.tool, extracted)
    assert sha(binary) == sha(extracted), 'Guest binary differs from host binary'

    status = private / 'Q3Status'
    ...
    command(xdf, disk, 'write', status, 'Q3/Q3Status')

    sequence = out / 'startup-sequence'
    sequence.write_text('FailAt 1\n'
        'Echo "AmigaOS 1.2 Q3: ' + args.tool + '"\n'
        'SYS:Q3/' + args.tool + ' >SYS:Q3/output.txt\n'
        'SYS:Q3/Q3Status >SYS:Q3/status.txt\n'
        'Echo >SYS:Q3/returned.txt "RETURNED BELOW FAILAT 1"\n'
        'Type SYS:Q3/output.txt\n'
        'Type SYS:Q3/status.txt\n'
        'Echo "Q3 command returned normally"\n')
    ...
    source_files = ['src/common/start_cli.S', 'src/common/start_cli.c',
        ...
        'ci/qualification/build-q3.sh', 'ci/qualification/run-q3.py']
    ...
    for name in ['output.txt', 'returned.txt', 'status.txt']:
        try:
            command(xdf, disk, 'read', 'Q3/' + name, out / name)
