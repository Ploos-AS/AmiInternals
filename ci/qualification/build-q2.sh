...
set -euo pipefail
cd "$(dirname "$0")/../.."
out="${1:-build/qualification/native-q2}"
mkdir -p "$out"
...
for spec in 'Ports:ports' 'Libs:libs' 'Devices:devices'; do
  tool="${spec%%:*}"
  dir="${spec##*:}"
...
sha256sum build/Ports build/Libs build/Devices | tee "$out/checksums.sha256"
echo 'PASS: native Q2 build and static startup/dependency checks' | tee "$out/result.txt"
